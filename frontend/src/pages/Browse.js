import React from 'react';
import "./Browse.css"
import ProductCard from "../components/product_card.js"

const defaultImage =
  "data:image/jpeg;base64,";

export const PRODUCTS = {
  hammer: { message: "Hammer", price: 13, item_img_url: defaultImage, reviewAvg: 3.2, reviewCount: 4 },
  "cooler-hammer": { message: "Cooler Hammer", price: 17, item_img_url: defaultImage, reviewAvg: 4.7, reviewCount: 9 },
  "coolest-hammer": { message: "Coolest Hammer", price: 19, item_img_url: defaultImage, reviewAvg: 4.9, reviewCount: 12 },
};

function Browse() {
  return(
    <div className="Browse">
      <header className="Browse-header">
        <h1>Browse SHOPNAO</h1>
        <div className="ProductGrid">
          <div className="ProductItem">
            <ProductCard id="hammer" message={PRODUCTS["hammer"].message} item_img_url={PRODUCTS["hammer"].item_img_url} price={PRODUCTS["hammer"].price} reviewAvg={PRODUCTS["hammer"].reviewAvg} reviewCount={PRODUCTS["hammer"].reviewCount}/>
          </div>
          <div className="ProductItem">
            <ProductCard id="cooler-hammer" message={PRODUCTS["cooler-hammer"].message} item_img_url={PRODUCTS["cooler-hammer"].item_img_url} price={PRODUCTS["cooler-hammer"].price} reviewAvg={PRODUCTS["cooler-hammer"].reviewAvg} reviewCount={PRODUCTS["cooler-hammer"].reviewCount}/>
          </div>
          <div className="ProductItem">
            <ProductCard id="coolest-hammer" message={PRODUCTS["coolest-hammer"].message} item_img_url={PRODUCTS["coolest-hammer"].item_img_url} price={PRODUCTS["coolest-hammer"].price} reviewAvg={PRODUCTS["coolest-hammer"].reviewAvg} reviewCount={PRODUCTS["coolest-hammer"].reviewCount}/>
          </div>
        </div>
      </header>
    </div>
  
  );
}


export default Browse;
